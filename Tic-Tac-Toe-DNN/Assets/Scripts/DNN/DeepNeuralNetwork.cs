...
namespace NN
{
    public class DeepNeuralNetwork
    {
        ...
        public DeepNeuralNetwork(int numInput, int[] numHidden, int numOutput)
        {
            ...
            this.hNodes = DeepNeuralNetwork.MakeJaggedMatrix(numHidden);
            this.oNodes = new double[numOutput];

            this.ihWeights = DeepNeuralNetwork.MakeMatrix(numInput, numHidden[0]);
            this.hhWeights = new double[numHidden.Length - 1][][];
            for (int i = 0; i < hhWeights.Length; i++)
            {
                this.hhWeights[i] = DeepNeuralNetwork.MakeMatrix(numHidden[i], numHidden[i + 1]);
            }
            this.hoWeights = DeepNeuralNetwork.MakeMatrix(numHidden[numHidden.Length - 1], numOutput);

            this.hBiases = DeepNeuralNetwork.MakeJaggedMatrix(numHidden);
        ...
        public void SetBiases(double[] bts)
        {
            ...
            if (nb != bts.Length)
            {
                throw new Exception("Bad bts[] length in SetBiases()");
            }
